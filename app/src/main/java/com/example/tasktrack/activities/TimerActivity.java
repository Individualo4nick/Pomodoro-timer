                ...
                switch (currentSelectedTimerMode) {
                    ...
                    case POMODORO:
                        if (status == TimerStatus.WORK) {
                            ...
                            Toast.makeText(TimerActivity.this, "Нет активных таймеров", Toast.LENGTH_SHORT).show();
                        }
                        break;
                }
            }
    ...
    private void initPomodoro() {
        ...
        pomodoroTimer.setWorkTimerEvents(new PomodoroTimer.CountDownTimerEvent() {
            ...
            public void onTick(long millisUntilFinished) {
                ...
                notifiyTimer("Task Tracker", "Время до перерыва: " + PomodoroTimer.getTimeString(millisUntilFinished), task.getId(), pomodoroTimer.getProgressBar().getProgress());
            }
